...
async function getAdminStats() {
  ...
  const utilization = facilities.map(f => {
    ...
    const percentage = Math.min(Math.round((bookingCount / maxCapacity) * 100), 100);
    
    let insight = 'Optimal usage';
    if (percentage > 80) insight = 'High demand - Consider adding similar resources';
    else if (percentage < 20) insight = 'Underutilized - Consider repurposing space';
...
async function getStudentStats(userId) {
  const [activeBookings, upcomingEvents, pendingBookings, lastBooking] = await Promise.all([
    prisma.booking.count({
      where: {
        userId,
        status: 'APPROVED',
  ...
  let suggestion = {
    ...
    message: 'Check out our new sports complex! It is less busy on Tuesday mornings.'
  };
  ...
  if (lastBooking) {
    ...
    suggestion = {
      title: 'Status Update',
      message: 'Your booking is being reviewed. Approvals usually take 24 hours.'
    };
  }
